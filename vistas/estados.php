					...
					<table class="table table-striped ">
					  ...
					  <tbody class="">
					    ...
					    <tr v-for="item in groceryList" v-bind:todo="item" v-bind:key="item.id">
					    	...
					    	<td class="align-middle">{{item.imagen}}</td>
	...
	var app7 = new Vue({
	  ...
	  methods: {
	    ...
	    eliminar_estado: function(item){
			...
			xhttp.open("DELETE", "http://localhost/hatch/organizador-proyectos/api/estado/"+item.id+"/", false);
			xhttp.send();
	    ...
	    agregar_estado: function(){
			...
			xhttp.open("POST", "http://localhost/hatch/organizador-proyectos/api/estado/", false);
			xhttp.send(formData);
